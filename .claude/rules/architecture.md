...
- `src/lib/` — Core utilities
  ...
  - `schema.ts` — Shared `orgJsonLd` (`Organization`, employer info nested as `worksFor`) and `personJsonLd` (`Person` with `@id`, `url`, `sameAs`, `knowsAbout`, etc.). The `@id` on `personJsonLd` is the canonical entity URI (`/me/`) that links all author occurrences across pages into one graph node — do not remove it.
  ...
  - `professional-years.ts` — `calculateYears({ year, month })` returns the number of full years elapsed since a start month; returns `Math.max(0, years)` so a future start date yields 0, not a negative number. Used by `me/page.tsx` and `me/llms.txt/route.ts` for `PROFESSIONAL_YEARS` and `PROGRAMMING_YEARS`.
