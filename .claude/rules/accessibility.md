...
- **SVGR strips `<title>` via SVGO**: `@svgr/webpack` runs SVGO by default, which removes `<title>` elements from SVG source files. This leaves `role="img"` SVGs with no accessible name (WCAG 1.1.1 violation). For decorative icon-only links (parent `<a>` already has a `sr-only` span), add `aria-hidden={true}` to the SVGR component — do not attempt to supply an accessible name via props.

- **WCAG 1.3.1 / 2.4.6 — h1 must appear before other content in `<main>`**: Every page must have its `<h1>` as the first DOM element in the main content area — before any links, buttons, or images. Screen readers announce elements in DOM order, so anything before the `<h1>` is read before the user can identify the page. Two past violations: `tag/[tag]/page.tsx` had a "← All Tags" `<Link>` before the `<h1>`, and `me/page.tsx` had `<PrintIconButton />` before the `<h1>`. The print button is `absolute`-positioned so moving it to the end of its `<section>` had no visual effect. Back-links and breadcrumbs belong *after* the `<h1>`, not before it.
...
- **WCAG 1.4.6 AAA — Enhanced contrast (7:1)**: AAA requires 7:1 vs AA's 4.5:1 for normal-weight text under 18pt. Safe Tailwind classes confirmed for this site's palette:
  ...
  - `text-violet-200` → 9.9:1 on `bg-slate-800` (active nav link on dark header; also the correct dark-mode pair for any interactive violet text — `dark:text-violet-400` only gives ~5.2:1 and **fails AAA**)
...
- **Use `slate-*` for all secondary text, not `gray-*`**: `text-gray-600` on `bg-slate-100` gives ~6:1 (fails AAA). Stick to the `slate-*` palette; the correct secondary text pair is `text-slate-700 dark:text-slate-300`.
